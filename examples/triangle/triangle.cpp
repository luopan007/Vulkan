...
// We want to keep GPU and CPU busy.
// To do that we may start building a new command buffer while the previous one is still being executed
...
public:
    ...
    // Pipelines (often called "pipeline state objects", PSO) are used to bake all states that affect a pipeline
    ...
    void createSynchronizationPrimitives() {
        ...
        VkSemaphoreCreateInfo semaphoreCI{}; // CI: CreateInfo
    ...
    void createVertexBuffer() {
        ...
        uint32_t vertexBufferSize = static_cast<uint32_t>(vertexBuffer.size()) * sizeof(Vertex); // 72 = 3 * 24

        // Setup indices
        std::vector<uint32_t> indexBuffer{ 0, 1, 2 };
        indices.count = static_cast<uint32_t>(indexBuffer.size());
        uint32_t indexBufferSize = indices.count * sizeof(uint32_t); // 12 = 3 * 4
