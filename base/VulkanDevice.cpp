...
namespace vks
{
    ...
    * 中文解释：虽然标准时统一的，但是各个厂家对于内存的实现时不同的，出于性能或者设计简单目的，memoryType数组中每一个都有若干种功能（即propertyFlags是组合值）。
    * 实际上，应用程序在使用时，只需要找到自己需要的是其子集，就代表满足要求；
    *
    * @param typeBits Bit mask with bits set for each memory type supported by the resource to request for (from VkMemoryRequirements) --- DDK返回的所支持的
    * @param properties Bit mask of properties for the memory type to request --- 应用程序额外添加的需求，比如：HOST_VISIABLE 或 HOST_COHERENT 等，反正是有特殊目的
    ...
    */
    uint32_t VulkanDevice::getMemoryType(uint32_t typeBits, VkMemoryPropertyFlags properties, VkBool32 *memTypeFound) const {
        std::cout << "typeBits: " << typeBits << std::endl;
        for (uint32_t i = 0; i < memoryProperties.memoryTypeCount; i++) {
            // typeBits： 是应程序将资源内存请求后，驱动返回的内存类型支持的位域集合，应用程序只需要检查该位为1里面的内存属性是否满足即可，若该位域不等于1，则一定不满足要求
            if ((typeBits & 1) == 1) {
                // properties：是应用程序针对该资源额外需要的内存属性要求
